...
Travel distance: %d]]
	...
	cargo_land_p2[4] = " are unloaded by a team of robotic drones supervised by a sentient overseer, who hands you your pay when they finish."
...
function create()
	...
	-- print ( string.format( "\n\tRegular Transport : entering create()" ) )
	...
	if ( seed > presence ) then
		-- print ( string.format( "\tRegular Transport : \tpresence check failed  (rnd=%i > presence=%i in system \"%s\")", seed, presence, system.cur():name() ) )
		misn.finish(false)
	else
		-- print ( string.format( "\tRegular Transport : \tpresence check success (rnd=%i < presence=%i in system \"%s\")", seed, presence, system.cur():name() ) )
	end
	...
	if destplanet == nil then
		-- print ( string.format( "\tRegular Transport : \tno suitable planet found (origin system \"%s\")", system.cur():name() ) )
		misn.finish(false)
	end
	...
	amount = rnd.rnd(1 + 25 * tier, 2 + 60 * tier)
	jumpreward = 200
	distreward = 0.09
	...
	-- print ( string.format( "\tRegular Transport : exiting create()" ) )
end
