...
if lang == "es" then
	 ...
	 desc = {}
	 desc[1] = "cargo delivery to %s"
	 desc[2] = "goods transport to %s"
	 desc[3] = "commodities move to %s"
	 desc[4] = "goods delivery to %s"
	 desc[5] = "commodities transport to %s"
	 desc[6] = "Commodities delivery to %s"
...
function create()
	...
	-- print ( string.format( "\tRush Transport : entering create()" ) )
	...
	if ( seed > presence ) then
		-- print ( string.format( "\tRush Transport : \tpresence check failed  (rnd=%i > presence=%i in system \"%s\")", seed, presence, system.cur():name() ) )
		misn.finish(false)
	else
		-- print ( string.format( "\tRush Transport : \tpresence check success (rnd=%i < presence=%i in system \"%s\")", seed, presence, system.cur():name() ) )
	end
	 ...
	 if destplanet == nil then
		-- print ( string.format( "\tRush Transport : \tno suitable planet found (origin system \"%s\")", system.cur():name() ) )
		 misn.finish(false)
	 end
	 ...
	 amount     = rnd.rnd(1 + 5 * tier, 3 + 10 * tier) -- 45 max (quicksilver)
	 jumpreward = 1000
	 distreward = 0.12
	 reward     = 1.5^tier * (numjumps * jumpreward + traveldist * distreward) * finished_mod * (1. + 0.05*rnd.twosigma())

	 misn.setTitle( buildCargoMissionDescription( desc, cargosize[tier], amount, cargo, destplanet, destsys ))
	...
	-- print ( string.format( "\tRush Transport : exiting create()" ) )
end
...
function land()
	if planet.cur() == destplanet then
		...
		if intime then
			...
			-- Semi-random message for being late and no reputation boost.
		...
		-- print ( string.format( "\tRush Transport : mission finished." ) )
		player.pay(reward)
		misn.finish(true)
	end
end
