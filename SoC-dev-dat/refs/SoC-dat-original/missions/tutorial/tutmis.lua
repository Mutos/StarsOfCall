...
if lang == "es" then
	 ...
	 "Okay, so let's try it for reals. The mission I'm sending you on is information gathering. Easy peasy. You just need to fly to the Crossover system, there's an old, abandoned station there. You need to fly to it and come to a stop close to it so your sensors can scan it. Once you do that, fly to Mhandari in the Nyotekundu system, I'll meet you there. Can you do that?"]]
	 ...
	 message2 = [[    "Great! Now, since you've accepted this mission, you will see your mission objectives printed on the screen when in space. Your current objective will be highlighted in green. Your first objective is to fly to the abandoned station. Like I said, it's in the Crossover system. I have put a marker next to that system on your galaxy map, so you know where to go. Missions tend to do this, so keep a look out. Once in Crossover, you can find the station with your overlay map.
	 ...
	 misn_desc = "You've been asked to fly to the abandoned station in the Crossover system and come to a stop close to it."
	 osd_title = "Tutorial mission"
	 osd_msg = { "Fly to the Crossover system",
					 "Come to a (near) stop close to the station",
					 "Wait until the scan is complete",
					 "Land on Mhandari in the Nyotekundu system"
				  }
end
...
function accept()
	 ...
	 marker = misn.markerAdd(system.get("Crossover"), "high")
	system.get("CL 0043"):setKnown(true, true)
	system.get("CL 0044"):setKnown(true, true)
	system.get("Crossover"):setKnown(true, true)
	system.get("Nyotekundu"):setKnown(true, true)
...
-- Land hook.
function land()
	 if planet.cur() == planet.get("Mhandari") and scanned then
...
-- Enter hook.
function enter()
	 if system.cur() == system.get("Crossover") then
		  misn.osdActive(2)
		  inrange = 0
...
function proxytrigger()
	 ...
	 if inrange == 7 then
		  ...
		  misn.markerMove(marker, system.get("Nyotekundu"))
		  scanned = true
		  misn.osdActive(4)
		  return
	 end
