...
function create ()
	...
	misn.setNPC( "Drunkard", "human/neutral/unique/drunkard" )  -- creates the drunkard at the bar
	misn.setDesc( bar_desc )           -- drunkard's description

	-- Planets
	pickupWorld, pickupSys  = planet.getLandable("Sanctuary")
	delivWorld, delivSys    = planet.getLandable("Inferno")
...
function takeoff()
	if system.cur() == delivSys and droppedoff then
		...
		willie = pilot.add( "Caravelle", "trader", player.pilot():pos() + vec2.new(-500,-500))[1]
