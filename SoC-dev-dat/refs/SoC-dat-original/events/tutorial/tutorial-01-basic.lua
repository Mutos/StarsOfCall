...
This tutorial will teach you what Naev is about, and show you the elementary controls of your ship.]]
	...
	message3 = [[Flying is easy, but stopping is another thing. To stop, you need to thrust in the direction you're heading in. To make this task easier, you can use %s to reverse your direction. Once you have turned around completely, thrust to decrease your speed. Try this now.]]
...
Land on Mars now. Remember, you can use the overlay map to get there quicker!]]
	message10 = [[Good job, you have landed on Mars. Your game will automatically be saved whenever you land. As a final tip, you can press %s even if you haven't targeted a planet or station - you will automatically target the nearest landable one.
...
Congratulations! This concludes the basic operation tutorial.]]
	...
	landomsg = "Target Mars with %s, then request landing permission with %s. Once granted, press %s again to land"
end
...
function create()
	...
	player.pilot():setPos(planet.get("Mars"):pos() + vec2.new(0, 250))
...
function input(inputname, inputpress)
	if waitmap and inputname == "overlay" then
		...
		boardee = pilot.add("Akk'Ti'Makkt", nil, targetpos)[1]
		boardee:disable()
