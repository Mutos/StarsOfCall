...
NPC missions are given to you by game characters. This most often happens in the spaceport bar, but sometimes a mission might come to you in other ways, so keep a look out.]]
	 message3 = [[We're going to accept a mission from an NPC. The NPC is waiting for you on Mhandari, in the spaceport bar. Land on Mhandari, then select the bar tab on the planet screen.]]
...
Congratulations! This concludes the missions and events tutorial.]]
	 ...
	 landomsg = "Land on Mhandari and visit the spaceport bar"
end

function create()
	 -- Set up the player here.
	player.teleport("Mhandari")
	 pilot.clear()
...
-- Land hook.
function land()
	 if planet.cur() == planet.get("Mhandari") then
		  if not (player.misnActive("Tutorial Mission") or player.misnDone("Tutorial Mission")) then
				tutNPC = evt.npcAdd("NPC", "Tutorial employee", "human/neutral/civilian/thief1", "This person has a mission for you!")
		  end
	 end
end
...
-- Helper land hook.
function bar()
	 if seen or planet.cur() ~= planet.get("Mhandari") then return
	 else
