...
You have been equipped with Ripper Cannons. To see how heat builds up in weapons, try firing into space (using %s) now. Keep an eye on your weapon indicators on the HUD - after a short while you should see the bars filling up red.]]
...
function create()
	...
	pp:rmOutfit("all")
	pp:addOutfit("Fighter Ripper Cannon", 4)
	pp:setDir(90)
	player.msgClear()
...
function flyUpdate()
	print(string.format ("\nEntering flyUpdate() from tutorial-07-heat.lua" ))
	print(string.format ("\tstage = %s", stage ))
	flytime = flytime - 1
	...
	elseif stage == stages.cool then
		...
		print(string.format ("Exiting flyUpdate() by launching cleanup() from tutorial-07-heat.lua" ))
		 cleanup()
		end
	end
	hook.timer(1000, "flyUpdate")
	print(string.format ("Exiting flyUpdate() from tutorial-07-heat.lua" ))
end
