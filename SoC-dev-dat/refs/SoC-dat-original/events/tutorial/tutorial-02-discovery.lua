...
A marker has been placed on your map. Navigate toward it. Remember, you can right-click on a location to get there faster.]]
	message4 = [[Something just happened. You discovered a planet in this system, called Hhhrish. Planets are the biggest objects you'll encounter in any system, so you will usually discover them before anything else.

Keep going.]]
	message5 = [[You've discovered something else, Hhhrish's habitable moon Ershiilt. Moons are a lot smaller than planets and are therefore harder to detect.
	
Some worlds, be they planets or moons, will be habitable. Note also that systems with habitable worlds are generally, but not always, be named from them.

You now know where Hhhrish and Ershiilt are located. Their positions have been added to your ship's computer, so you won't have to search for them again. Even if you fly away from them again, you will still be able to see them on the map.

There is still one thing left in this system for you to find: the jump point leading to the next system. Jump points are generally much more difficult to find than planets or moons, especialy in a high interference system such as this one. Look at your map, the marker is now toward the upper side. Fly there.]]
...
- Keep a look out for Star Maps. These maps are sold on planets, and each one reveals a specific part of the galaxy. Most maps reveal several systems at once, and will usually reveal some or all of their jump points as well. When you begin, your first ship already has a standard map of the faction on which capitol you bought it, showing only that faction's major systems and routes.
...
function create()
	...
	targetplanet = planet.get("Hhhrish")
	targetmoon = planet.get("Ershiilt")
	jmp = jump.get(system.cur(), "Bi 0040")
...
function input(inputname, inputpress)
	if inputname == "overlay" then
		...
		marker = system.mrkAdd("Fly here", targetplanet:pos())
...
function discover(disctype, discovered)
	...
	if disctype == "asset" and discovered == targetplanet then
		tk.msg(title1, message4)
	elseif disctype == "asset" and discovered == targetmoon then
